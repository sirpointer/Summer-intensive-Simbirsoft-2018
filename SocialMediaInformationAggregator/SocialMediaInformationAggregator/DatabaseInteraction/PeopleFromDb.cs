...
namespace SocialMediaInformationAggregator.DatabaseInteraction
{
    public static class PeopleFromDb
    {
        ...
        {
            /// возвращает данные о найденных людях для определенного пользователя
            ConnectToDatabase();
            SqlConnection conn = new SqlConnection(connectionString);
            conn.Open();
            string query = String.Format("SELECT * FROM FoundPersons WHERE UserLogin=@a");
        ...
        public static List<string> GetRowInformFromDb(string login, int k)
        {
        ...
        public static List<string> SeparateElements(List<string> mas)
        {
            ...
            {
                List<string> buf = a.Split(',').ToList();
                foreach (string p in buf)
                    elements.Add(p);
            }
        ...
        public static List<string> EducationFoundPeople(string login, int count = -1, string startWith = "")
        {
            ...
            if (!String.IsNullOrWhiteSpace(login) && (count > 0 || count == -1))
            {
                if (count > 0 && startWith != "")
                {
                    ...
                    education = SeparateElements(AllElements).Where(a => a.StartsWith(startWith)).OrderBy(a => a).Take(count).ToList();
                }
                else if (count == -1 && startWith != "")
                {
                    List<string> AllElements = GetRowInformFromDb(login, 5);
                    IEnumerable<string> res = SeparateElements(AllElements).Where(a => a.StartsWith(startWith)).OrderBy(a => a);
                    education = res.ToList();
                }
                else if (count == -1 && startWith == "")
                {
                    List<string> AllElements = GetRowInformFromDb(login, 5);
                    List<string> smth = SeparateElements(AllElements);
                    IEnumerable<string> res = smth.OrderBy(a => a);
                     education = res.ToList();
                }
                else if (count> 1 && startWith == "")
                {
        ...
        public static void AddFoundPerson(FindPeople.PersonInformation pers)
        {
                ...
                && !String.IsNullOrEmpty(pers.YearOfBirth.ToString()))
            {
                ...
                if (buf == -1)
                {
                    ...
                    SqlConnection conn = new SqlConnection(connectionString);
                    conn.Open();
                    string query = String.Format("INSERT INTO FoundPersons " +
        ...
        public static string MakeEducationRow(FindPeople.PersonInformation pers)
        {
            string education = "";
        ...
        public static string MakeCityRow(FindPeople.PersonInformation pers)
        {
            string cities = "";
            foreach (string a in pers.Education)
            {
